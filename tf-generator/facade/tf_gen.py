...
def _generate_alb_ingress_controller(config):
    ...
    # the following code is  how to generate alb output, left commented till the necessary info is gotten
    #Get alb id and state
    #  output_alb_id = TFStringBuilder.generate_output("alb_id", alb_id, description="ALB ID")
    #  output_alb_state = TFStringBuilder.generate_output("alb_state", alb_state, description="ALB State")

    #return output_alb_id + output_alb_state
    pass
...
def _generate_vpc_resource(config):
    ...
    vpc_config = {
     ...
     # Generate output for VPC ID and its state
    output = TFStringBuilder.generate_output("vpc_id", "aws_vpc.vpc_{config['aws_region']}.id", description="VPC ID")
    output += TFStringBuilder.generate_output("vpc_state", "aws_vpc.vpc_{config['aws_region']}.state", description="VPC State")
...
def _generate_subnet_resources(config):
    ...
    for i, subnet in enumerate(subnets):
        ...
        subnet_index = i
        output_subnet_id = TFStringBuilder.generate_output(f"subnet_{subnet_index}_id", f"aws_subnet.subne{subnet_index}.id", description=f"Subnet {subnet_index} ID")
        builder += output_subnet_id
            
            # Add output block for subnet state
        output_subnet_state = TFStringBuilder.generate_output(f"subnet_{subnet_index}_state", f"aws_subnet.subnet_{subnet_index}.state", description=f"Subnet {subnet_index} State")
        builder += output_subnet_state
            
            # Add output block for availability zone
        output_availability_zone = TFStringBuilder.generate_output(f"subnet_{subnet_index}_availability_zone", subnet['availability_zone'], description=f"Subnet {subnet_index} Availability Zone")
